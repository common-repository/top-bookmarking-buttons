<?php
/*
Plugin Name: Top Bookmarking Buttons
Version: 0.1
Plugin URI: http://wordpress.org/extend/plugins/top-bookmarking-buttons/
Description: The plugin contains Top Bookmarking buttons on the NET. Post all your content to the social bookmarking sites to get traffic for your website or blog. Visit <a href="http://socibook.com" target="_blank">plugin page</a>.
Author: Biser Markov
...
function add_me($content) {
	...
	$my_images_folder = get_settings('home') . '/wp-content/plugins/top-bookmarking-buttons/images/';
	
	$my_link = get_permalink($post->ID);	
    $my_title = get_the_title($post->ID);

	if ( !is_feed() && !is_page() ) {
		$content .= "\n\n" . '<!-- Begin Top Bookmarking Buttons plugin -->' . "\n"
                  . '<!-- http://wordpress.org/extend/plugins/top-bookmarking-buttons/ -->' . "\n"
                  ...
                  . '</div>' . "\n"
				  . '<!-- End Top Bookmarking Buttons plugin -->' . "\n\n";				  
    }				  
	return $content;
}
